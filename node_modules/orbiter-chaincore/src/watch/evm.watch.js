...
class EVMWatchBase extends base_watch_1.default {
    constructor(chain) {
        super(chain);
        this.chain = chain;
            ...
            try {
                ...
                while (start <= end) {
                    try {
                        ...
                        config.debug &&
                            this.logger.debug(`[${config.name} - replayBlock - GgetBlockBefore] Block:${start}/${end},, timestamp:${timestamp}`);
                        const block = yield web3.eth.getBlock(start, true);
                        if (!block) {
                            this.logger.error(`[${config.name} - Get empty block`);
                            break;
                        }
                        config.debug &&
